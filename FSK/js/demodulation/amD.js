...
function calcSineYAMD(x, factor) {
    ...
    var aCar=30;
    var fc2 = document.getElementById('c-amplitude-d').value;
    var fm = document.getElementById('m-frequency-d').value/1000;
    var fc = document.getElementById('c-frequency-d').value;

    var msgSig= aMsg*Math.sin( 2 * Math.PI * (fm/w) * x );
    if(msgSig>0){
      return h-(aCar* Math.sin( 2 * Math.PI * (fc/w) * x ));
    }  
    else{
      return h-(aCar* Math.sin( 2 * Math.PI * (fc2/w) * x ));
    }
